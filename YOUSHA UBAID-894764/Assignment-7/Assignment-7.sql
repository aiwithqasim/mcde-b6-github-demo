...
-- Sequential row number for all products by price
...
--When ORDER BY is used, the default window frame is:

--RANGE BETWEEN UNBOUNDED PRECEDING AND CURRENT ROW

--FIRST_VALUE() works correctly because the first row is always included in the default frame.

--But LAST_VALUE() only looks up to the current row because the default frame ends at CURRENT ROW. Therefore, it does not see the actual last row of the partition.

--To make LAST_VALUE() see the entire partition, we use:

--RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING

--In short:
--FIRST_VALUE() → first value is already visible in the default frame.
--LAST_VALUE() → needs the frame extended to UNBOUNDED FOLLOWING to see the actual last value.