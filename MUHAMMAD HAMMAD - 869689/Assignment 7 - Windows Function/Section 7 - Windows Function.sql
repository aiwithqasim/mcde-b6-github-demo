...
--7.4 - Use NTILE(5) to divide all products into five price
--bands. Return the product name, price, and band number.
...
--7.5 - Write a query that shows each order with a runningtotal of revenue
...
FROM order_revenue
ORDER BY order_date, order_id;
--7.6 - Think About It: Why does LAST_VALUE() require RANGE BETWEEN UNBOUNDED PRECEDING AND UNBOUNDED FOLLOWING to 
--return the actuallast value in the partition, while FIRST_VALUE() works correctly with the default frame? 
...
FIRST_VALUE() works correctly because the first row is included in the frame.
LAST_VALUE() returns thelast value of the current frame,not the entire partition.
Therefore, UNBOUNDED FOLLOWING is required to include the last row of the partition andreturn the actual last value.*/