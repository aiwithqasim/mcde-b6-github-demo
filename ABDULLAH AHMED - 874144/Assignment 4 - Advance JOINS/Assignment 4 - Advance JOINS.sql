/* SELF JOIN
 ...
 List each staff member alongside their manager's full name. If a staff member has no manager (top-level), still show 
 them with NULL for manager name./*
...
LEFT JOIN sales.staffs AS m
    ...
    /* TASK 42
     Find pairs of products from the same brand that have the exact same list price. Show both product names and the 
     brand name.*/
...
/* TASK 46
   Using the result of a CROSS JOIN between brands and categories, find brand-category combinations that have 
   NO products (LEFT JOIN the cross join result against products and filter for NULLs).*/
...
Hint: LEFT JOIN production.products with sales.order_items, filter WHERE order_id IS NULL.Find all products that are NOT 
     currently in stock at ANY store.*/
