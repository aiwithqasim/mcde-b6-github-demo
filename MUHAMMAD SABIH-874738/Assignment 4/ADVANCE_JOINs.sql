...
--find brand-category combinations that have NO products (LEFT JOIN the cross join result against products and filter for NULLs).
SELECT
b.brand_name,
c.category_name
FROM production.brands b
...
ss.store_name
FROM sales.staffs s
CROSS JOIN sales.stores ss
LEFT JOIN sales.stores actual
ON s.store_id = actual.store_id;

--Right Join

--Task 49: List all brands and the products that belong to them. Ensure ALL brands appear,
--even if they have no products. Use a RIGHT JOIN (products RIGHT JOIN brands).
SELECT 
b.brand_name,
p.product_name
FROM production.products p
